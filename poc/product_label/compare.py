...
def main_golden() -> None:
    ...
    L.append("")
    L.append("**정답 기준 (골든 1차 판정 공통 — 12장 확정 프롬프트 정의에 경계 사례를 더함, 검토 필요)**")
    L.append("")
    L.append("| 구분 | 기준 |")
    L.append("|---|---|")
    L.append("| 제품 라벨 | 사진·제품 렌더에 찍힌 물건 표면의 글자 — 용기·패키지 인쇄, 제품 표면 스티커·손글씨, 반사·좌우반전 글자 |")
    L.append("| 배경 텍스트 | 페이지에 얹은 글자 — 헤드라인·설명·각주, 배지·버튼·말풍선 안 문구, 제품 사진 옆·위에 얹은 제품명·배지, 페이지 상단 로고 |")
    L.append("| 캡처 문서 | 영수증·앱 화면·논문·인증서 캡처 안 글자는 배경 |")
    L.append("| 혼합 블록 | 라벨 글자가 조금이라도 섞이면 정답은 라벨(미탐 회피) |")
    L.append("| 비텍스트 오검출 | 얼굴·아이콘 등 글자 아닌 블록은 미탐·오탐 세지 않음 — 정답지에는 판정 그대로 남음 |")
    L.append("| 애매 | 미탐 회피 원칙대로 라벨로 봄, 비고에 사유 |")
    L.append("")
