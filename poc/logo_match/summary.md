...
| variant | 방식 | 임계 | 소요 |
|---|---|---|---|
| `template_gray` | 다중 스케일 밝기 매칭(NCC) | 점수 ≥ 0.8 | 91.67s |
| `template_edge` | 윤곽선끼리 매칭 | 점수 ≥ 0.45 | 87.44s |
| `feature_orb` | ORB 특징점 + 호모그래피 | 인라이어 ≥ 12 | 4.22s |

템플릿 — `b.clinicx` · `goodal`. 배율 0.4~2.0배. `results/_templates/`에 저장.

## 2. 확정 오탐 (기계 집계)

**남의 브랜드 페이지에서 임계를 넘은 매칭.** 정답 없이도 오탐임이 확실함.

| variant | `b.clinicx` 확정 오탐 | `goodal` 확정 오탐 | 합계 | 자기 페이지 통과 |
|---|---|---|---|---|
| `template_gray` | 0 | 0 | **0** | 2 |
| `template_edge` | 0 | 0 | **0** | 2 |
| `feature_orb` | 0 | 0 | **0** | 0 |
